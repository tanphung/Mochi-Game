# Mochi — On-chain AI Pet dApp on GenLayer
...
## 👀 About
**Mochi** is a GenLayer intelligent-contract dApp where each wallet raises an on-chain AI pet companion. Feed, play, clean and chat with your pet (replies generated by an on-chain LLM under validator consensus), customize its look and room, and mint shareable cards. It also includes a **Quest Evaluator** that reads your real submission links (including X/Twitter posts) and uses GenLayer's web access + LLM to check them against quest requirements before you submit.

## 🔗 Demo
- **Live app:** _add your Vercel link here_
- **Deployed contract (GenLayer Studio):** `0xFb3bb2eE4eBD8e2ff7568984FD84594D0D2E2F1e`

## 📦 What's included
- An intelligent contract for the Mochi pet (`MochiPet`) covering pet care, an on-chain LLM chat, customization, NFT-style cards, and the Quest Evaluator
- Example end-to-end tests for the contract (`/test`) and a deploy script (`/deploy`)
- A configuration file template (`/frontend/.env.example`)
- A production-ready Next.js frontend with TypeScript, TanStack Query, and Radix UI
...
6. Quest Evaluator (AI + live web access):
   - `evaluate_quest` fetches each evidence link via GenLayer's web access (`gl.nondet.web.get`) — X/Twitter posts are read through the public oEmbed API — then asks an on-chain LLM to judge the submission against the pasted quest requirements. A custom leader/validator pair validates the structured JSON result before it is stored, so the verdict survives consensus.
